# Fast-Browser-MCP Navigator — Chrome headless + MCP SSE server
# Build:  docker build -t fast-browser-mcp .
# Run:    docker run -p 8000:8000 fast-browser-mcp
# With key: docker run -p 8000:8000 -e MCP_API_KEY=secret fast-browser-mcp
...
CMD ["python", "-m", "fast_browser_mcp", "--transport", "sse"]
